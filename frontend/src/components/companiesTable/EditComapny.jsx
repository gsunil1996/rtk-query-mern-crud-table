...
import { useGetSingleCompanyDetailsQuery, useUpdateCompanyDetailsMutation } from '../../redux/features/companiesSlice';
...
const EditComapny = (props) => {
    ...
    const { editCompanyopen, setEditCompanyOpen, tableRowId } = props;
    ...
    const [updateCompany, { isLoading, isSuccess, isError, error, reset }] = useUpdateCompanyDetailsMutation();

    const { data: singleCompanyDetails, isFetching: singleCompanyFetching, isError: singleCompanyIsError, error: singleCompanyError, isSuccess: singleCompanySuccess } = useGetSingleCompanyDetailsQuery(tableRowId ? tableRowId : skipToken);
    ...
    useEffect(() => {
        if (singleCompanyDetails?.fname) {
            setInputData(singleCompanyDetails)
        }

    }, [tableRowId, editCompanyopen, singleCompanyDetails])
    ...
    return (
        <div>
            ...
            >
                ...
                {singleCompanyFetching ? (
                    <div style={{ width: "100%", marginTop: "20px" }}>
                        <LinearProgress />
                    </div>
                ) : singleCompanyIsError ? (
                    <div style={{ width: "100%", marginTop: "20px", textAlign: "center" }}>
                        <h1>{singleCompanyError?.status}</h1>
                    </div>
