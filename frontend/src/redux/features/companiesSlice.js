...
export const companiesSlice = apiSlice.injectEndpoints({
  endpoints: (builder) => ({
    getcompaniesTable: builder.query({
      query: ({ search, gender, status, sort, page }) =>
        `/companiesTable?search=${search}&gender=${gender}&status=${status}&sort=${sort}&page=${page}`,
...
export const {
  useGetcompaniesTableQuery,
  useAddComapnyDataMutation,
